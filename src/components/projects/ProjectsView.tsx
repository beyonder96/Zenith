...
export function ProjectsView({ projects, loading, searchTerm }: ProjectsViewProps) {
            ...
            .catch(error => {
                if (error.code === 'permission-denied') {
                    ...
                    toast({
                        ...
                        description: `Não foi possível marcar a tarefa. Detalhe: ${error.message}`
                    });
                }
            });
    };
          ...
          deleteDoc(docRef).then(() => {
              ...
              if (error.code === 'permission-denied') {
                  ...
                  toast({
                      ...
                      description: `Não foi possível deletar o projeto. Detalhe: ${error.message}`
                  });
              }
              setProjectToDelete(null);
          });
            ...
            setShowDeleteCompletedDialog(false);
             ...
             if (error.code === 'permission-denied') {
                ...
                toast({
                    ...
                    description: `Não foi possível limpar as tarefas. Detalhe: ${error.message}`
                });
            }
            setShowDeleteCompletedDialog(false);
        });
    ...
    const handleAiSplit = async (id: string) => {
        ...
        try {
            ...
            if (error.code === 'permission-denied') {
                const permissionError = new FirestorePermissionError({ path: `projects/${id}`, operation: 'update' });
                errorEmitter.emit('permission-error', permissionError);
            } else {
                toast({ variant: "destructive", title: "Erro de Conexão", description: `Não foi possível conectar ao serviço de IA. Detalhe: ${error.message}` });
            }
        } finally {
                ...
                if (error.code === 'permission-denied') {
                    ...
                    errorEmitter.emit('permission-error', permissionError);
                     ...
                     toast({
                        variant: "destructive",
                        title: "Erro ao atualizar",
                        description: `Não foi possível atualizar a subtarefa. Detalhe: ${error.message}`
                    });
                }
            });
    };
